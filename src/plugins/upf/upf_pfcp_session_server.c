...
pfcp_session_server_rx_callback (session_t * s)
{
  ...
  rv = svm_fifo_peek (s->rx_fifo, ph.data_offset + SESSION_CONN_HDR_LEN, len, msg->data);
  ...
  vlib_process_signal_event_mt (gtm->vlib_main, pfcp_api_process_node.index, EVENT_RX,
				(uword) msg);
